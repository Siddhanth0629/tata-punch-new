document.addEventListener("DOMContentLoaded", (event) => {
  ...
  rgbeLoader.load("./assets/parkingLot2.hdr", function (texture) {
  ...
  //fifth aniamtion
  // camera.position.set(-0.02, 1.856, 4.082);
  // camera.rotation.set(-0.476, 0.007, 0.007);
  ...
  function cleanUpRenderer() {
    ...
    renderer.dispose();
    renderer.forceContextLoss(); // 🔥 Releases GPU memory
    renderer.domElement.remove(); // 🔥 Removes from DOM
  }
  ...
  window.addEventListener("resize", () => {
    ...
    }, 250); // Throttle to avoid multiple calls
  });
  ...
  // cameraFolder.add(camera.position, "z", -5, 5).name("Position Z").step(0.001);
